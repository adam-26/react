...
export default class PluginManager {
  ...
  addPlugin(plugin: FrameBoundaryPlugin): void {
    ...
    this._pluginNames.push(plugin.pluginName);
  }
  ...
  renderFrameBoundary(
    ...
    if (!this.hasPlugins || pluginNames.length === 0) {
      ...
      return renderElement(element, frame.context, frame.domNamespace);
    }
    ...
    const renderFrameWithPlugin = (element: ReactNode, context: Object, domNamespace: string) => {
      ...
      return this._plugins[nextPlugin].renderFrameBoundary(
        ...
        domNamespace, {
          ...renderUtils,
          renderElement: remainingPlugins.length ? renderFrameWithPlugin : renderElement
        });
    };
